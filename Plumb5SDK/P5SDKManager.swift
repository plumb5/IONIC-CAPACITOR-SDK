...
//        params.
    ...
    class func getPushCampaignForBLE(region: P5BeaconRegion, radius: String, entry:String){
        var offLine = Constants.value_offlineMode
        if P5SDKManager.sharedInstance.checkNetworkRecheablity(){
            offLine = Constants.value_onlineMode
        }
        P5DBStoreManager.sharedInstance.addBeaconEeventData(beaconName: region.beaconName, beaconId: region.uuid, type: entry, offline: offLine)
        
        var params : Dictionary<String, String> = Dictionary()
        params.updateValue(region.pushId, forKey: Constants.key_PushId)
        params.updateValue(entry, forKey: Constants.key_Entry)
        params.updateValue(P5SDKManager.sharedInstance.p5getAppKey, forKey: Constants.key_AppKey)
        params.updateValue(P5SessionId.sharedInstance.getSessionId(), forKey: Constants.key_SessionId)
        params.updateValue(P5DeviceInfo.sharedInstance.getDeviceId(), forKey: Constants.key_DeviceId)
        P5SDKManager.sharedInstance.getGeoFencePushCampiagn(params: params,beaconName: region.beaconName,geofenceName: "")
    }
...
//            P5SDKManager.sharedInstance.delegate = nil;
            ...
            Utility.setAutolayout(toView: p5BannerView, inView: bannerView, attributes: "0,0,0,0", fieldPosition: .None, topView: UIView(), topViewAttibutes: "0,0,0,0", width: 0, alignment: "Left")
