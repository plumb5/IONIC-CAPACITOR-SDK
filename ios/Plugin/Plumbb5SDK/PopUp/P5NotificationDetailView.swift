...
class P5NotificationDetailView: UIView {
    ...
    func drawNotificationBanner() {
        ...
        let imgClose = UIImage(named: "ic_close", in: frameWorkBundle, compatibleWith: nil)
        ...
        if attachmentUrl != nil && (attachmentUrl?.count)! > 0 {
            ...
            viewContainer.addSubview(mediaView)
            var verticalVisulaStringForMedia = "V:|-[mediaView]"
        ...
        P5PushNotificationsHandler().fromeData(type: "view", MobileFormId: String(describing: MobilePushId!), WidgetName: widgetName!, WorkFlowDataId: widgetName)
    ...
    func createPausePlayButton(inView: UIView) {
        ...
        let frameWorkBundle = Bundle(for: type(of: self))
        let imgPlay = UIImage(named: "play", in: frameWorkBundle, compatibleWith: nil)
        let imgPause = UIImage(named: "pause", in: frameWorkBundle, compatibleWith: nil)
    ...
    func downloadImageWithUrl(urlString: String, imageView: UIImageView) {
        ...
        if P5SDKManager.sharedInstance.cache.object(forKey: urlString as AnyObject) != nil {
            ...
            let height = getHeightForWidth(width: imageView.frame.size.width, size: (imageView.image?.size)!)
    ...
    func getHeightForWidth(width _: CGFloat, size: CGSize) -> CGFloat {
        var requiredHeight = 0.0
        //        let aspectRatio = size.width/size.height;
        requiredHeight = Double(((frame.size.width - 14) * size.height) / size.width)
        return CGFloat(requiredHeight)
    }

    @objc func formCloseTapped() {
        removeFromSuperview()
        P5PushNotificationsHandler().fromeData(type: "close", MobileFormId: String(describing: MobilePushId!), WidgetName: widgetName!, WorkFlowDataId: "")
        removeAllOberser()
    }
    ...
    func createAVPlayerWithParams(mediaLink: String, inView: UIView) {
        player = AVPlayer(url: URL(string: mediaLink)!)
        ...
        playerLayer.frame = CGRect(x: 0, y: 0, width: frame.size.width - 14, height: 200)
    ...
    override func touchesBegan(_ touches: Set<UITouch>, with _: UIEvent?) {
        if let touch = touches.first {
            ...
            if formView.frame.contains(currentPoint) {
                ...
                P5PushNotificationsHandler().fromeData(type: "click", MobileFormId: String(describing: MobilePushId!), WidgetName: widgetName!, WorkFlowDataId: "")
