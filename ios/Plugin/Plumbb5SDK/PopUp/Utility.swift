...
class Utility {
    ...
    public class func setboarderToView(view: UIView, fromParams: NSDictionary, fillBg: Bool) {
        ...
        if Border.count >= 0 {
            view.layer.borderColor = UIColor(hexString: Border).cgColor
        }
    ...
    public class func convertWidthPercentageToPixel(width: CGFloat, inView: UIView) -> CGFloat {
        ...
        let screenWidth = inView.frame.size.width // UIApplication.shared.keyWindow?.bounds.size.width
    ...
    public class func convertHeighrPercentageToPixel(width: CGFloat) -> CGFloat {
        ...
        let screenHeight: CGFloat = (UIApplication.shared.keyWindow?.bounds.size.height)!
    ...
    class func calculateVerticalGap(topViewAttibutes: String?, topMargin: String) -> String {
        ...
        if topViewAttibutes != nil {
            ...
            if array?.count == 4 {
                let gap = CGFloat((array![3] as NSString).doubleValue) + CGFloat((topMargin as NSString).doubleValue)
    ...
    public class func setAutolayoutToIndependentField(toView: UIView, inView: UIView, attributes: String, width: CGFloat, alignment: String?, fieldPosition _: FieldPosition) {
        let newattributes = Utility.convertPixels(positions: attributes)
        toView.translatesAutoresizingMaskIntoConstraints = false
        let array = newattributes.components(separatedBy: ",")
    ...
    public class func convertPixels(positions: String) -> String {
        ...
        let comps: [String] = positions.components(separatedBy: ",")
        if comps.count == 4 {
            //            let p1 = CGFloat((comps[0] as NSString).doubleValue)
            var newPositionsArr: [String] = Array()
