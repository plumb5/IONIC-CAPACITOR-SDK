...
//        closeBtn.isHidden = true;
    ...
    @objc func downloadImageWithUrl(urlString: String, imageView: UIImageView) {
        ...
        if P5SDKManager.sharedInstance.cache.object(forKey: urlString as AnyObject) != nil {
            ...
            P5PushNotificationsHandler().fromeData(type: "view", MobileFormId: String(describing: MobilePushId!), WidgetName: widgetName!, WorkFlowDataId: "")
            activityIndicator?.stopAnimating()
            activityIndicator?.removeFromSuperview()
            closeBtn.isHidden = false
            if delegate != nil {
                delegate?.bannerLoaded()
            }
        } else {
            // 3
            ...
            task = session.downloadTask(with: url, completionHandler: { _, _, _ in
                if let data = try? Data(contentsOf: url) {
                    ...
                    DispatchQueue.main.async { () in
                        ...
                        P5PushNotificationsHandler().fromeData(type: "view", MobileFormId: String(describing: self.MobilePushId!), WidgetName: "Banner", WorkFlowDataId: "")
